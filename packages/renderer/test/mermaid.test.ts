 ...
 *
 * The renderer emits ```mermaid fences as a standard
 * `<pre><code class="language-mermaid">` block carrying the diagram source as
 * escaped text — the same shape GitHub's static HTML ships before its
 * client-side mermaid run. Hydration into SVG is a browser concern:
...
describe('mermaid placeholders (task 1.7)', () => {
  it('emits an inert language-mermaid code block for ```mermaid fences', async () => {
